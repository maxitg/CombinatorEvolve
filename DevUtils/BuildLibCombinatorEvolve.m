...
PackageExport["BuildLibCombinatorEvolve"]

Options[BuildLibCombinatorEvolve] = {
  "RepositoryDirectory" :> $CombinatorEvolveRoot,
...
SyntaxInformation[BuildLibCombinatorEvolve] =
  {"ArgumentsPattern" -> {OptionsPattern[]}, "OptionNames" -> Options[BuildLibCombinatorEvolve][[All, 1]]};

SetUsage @ "
BuildLibCombinatorEvolve[] builds the libCombinatorEvolve library from source, and returns an association of metadata \
...
* By default, the sources are obtained from the 'libCombinatorEvolve' subdirectory of the current repo, but this \
...
* Additional metadata is written to 'LibraryTargetDirectory' in a file called 'libCombinatorEvolveBuildInfo.json'.
...
BuildLibCombinatorEvolve::compfail = "Compilation of C++ code at `` failed.";
BuildLibCombinatorEvolve::badsourcedir = "Source directory `` did not exist.";

BuildLibCombinatorEvolve[OptionsPattern[]] := ModuleScope[
  (* options processing *)
  UnpackOptions[
  ...
  SetAutomatic[librarySourceDirectory, FileNameJoin[{repositoryDirectory, "libCombinatorEvolve"}]];
  SetAutomatic[libraryTargetDirectory, FileNameJoin[{repositoryDirectory, "LibraryResources", systemID}]];

  (* path processing *)
  buildDataPath = FileNameJoin[{libraryTargetDirectory, "libCombinatorEvolveBuildInfo.json"}];
  ...
  (* derive final paths *)
  libraryFileName = StringJoin["libCombinatorEvolve-", finalHash, ".", System`Dump`LibraryExtension[]];
  ...
  If[!StringQ[libraryPath],
    Message[BuildLibCombinatorEvolve::compfail, librarySourceDirectory];
...
$printPreBuildCallback = Function[Print["Building libCombinatorEvolve from sources in ", #LibrarySourceDirectory]];
