...
PackageExport["PackCombinatorEvolve"]

Options[PackCombinatorEvolve] = {
  "RepositoryDirectory" :> $CombinatorEvolveRoot, (* for Git SHAs etc *)
  "SourceDirectory" -> Automatic, (* for WL sources *)
  "LibraryDirectory" -> Automatic, (* for libCombinatorEvolve *)
  "MasterBranch" -> "master", (* for calculating the minor version *)
  "OutputDirectory" -> Automatic,
  "Verbose" -> False
};

PackCombinatorEvolve::nosources = "There were no .m files in ``.";
PackCombinatorEvolve::nolibraries = "There were no library files in ``.";
PackCombinatorEvolve::packfailed = "Could not pack paclet from `` into ``.";
PackCombinatorEvolve::nopacletinfo = "Paclet info file was not present at ``.";
PackCombinatorEvolve::nobuildinfo = "Build info file was not present at ``.";
PackCombinatorEvolve::nogitlink = "GitLink is not installed, so the built paclet version cannot be correctly \
calculated. Proceed with caution, and consider installing GitLink by running InstallGitLink[].";

SyntaxInformation[PackCombinatorEvolve] =
  {"ArgumentsPattern" -> {OptionsPattern[]}, "OptionNames" -> Options[PackCombinatorEvolve][[All, 1]]};

SetUsage @ "
PackCombinatorEvolve[] creates a PacletObject containing the local source and last built library.
* Note that PackCombinatorEvolve[] does *not* call BuildLibCombinatorEvolve[], unlike the command line scripts.
...
* The built libCombinatorEvolve libraries are taken from the LibraryResources subdirectory of 'LibraryDirectory'.
...
PackCombinatorEvolve[OptionsPattern[]] := ModuleScope[
  ...
  ,
    Message[PackCombinatorEvolve::nogitlink];
